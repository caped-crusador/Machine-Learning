...
def linear_regression_Ridge_momentum(predictor_vals, target_vals, lr, repetitions, lam, beta):
    ...
    if shape(predictor_vals) == (21613, 15):
        ...
        show()
        loglog(L)
        title("Loss Decline after gradient Descent)")
        xlabel("Iterations")
        ylabel("Loss")
...
p1, p2, p3, loss = linear_regression_Ridge_momentum(predictor_vals=X_1, target_vals=Y, lr=0.000008, repetitions=10000, lam=0, beta=0.91)
y_1 = predict(param1=p1, param2=p2, param3=p3, x=X_1)
plot(X_1, Y, ".", color="Orange")
plot(X_1, y_1, ".", label="predicted")
title("Actual vs Predicted (learning rate = 0.00008)[on X]")
legend()
xlabel("Size (sqft)")
ylabel("Price ($)")
show()
...
# For X
parameters, loss2 = linear_regression_Ridge_momentum(predictor_vals=X, target_vals=Y, lr=0.00009, repetitions=1000, lam=0.01, beta=0.5)
...
p1, p2, loss = linear_regression_Ridge_momentum(predictor_vals=X_1, target_vals=Y, lr= 0.00009, repetitions=10000)
